# -*- coding: utf-8 -*-
"""
🎯 SCRIPT COMPLETO Y CORREGIDO: MAPA DE ELEMENTOS EXPUESTOS (SISTEMA PIURA)
...
✅ NUEVO: AGREGADA CAPA DE LÍMITES DEPARTAMENTAL Y PROVINCIAL SIN AFECTAR EL ZOOM.
"""
...
def generar_mapa_elementos_expuestos(nombre_usuario, departamento_sel, provincia_sel, distrito_sel):
    ...
    print("\n" + "="*80)
    print("🗺️ INICIANDO GENERACIÓN DE MAPA DE ELEMENTOS EXPUESTOS (SISTEMA PIURA)")
    print("="*80)
    ...
    if not os.path.exists(ruta_distritos_correcta):
        ...
        print(f"   {ruta_distritos_correcta}")
        return None
    
    try:
        gdf_distritos = gpd.read_file(ruta_distritos_correcta)
        if gdf_distritos.crs is None or gdf_distritos.crs.to_epsg() != 4326:
            gdf_distritos = gdf_distritos.set_crs(epsg=4326)
        gdf_distritos = gdf_distritos.to_crs(epsg=3857)
        
    except Exception as e:
        print(f"❌ Error cargando shapefile de distritos: {e}")
        traceback.print_exc()
        return None
    ...
    # ╔═══════════════════════════════════════════════════════════════════════════╗
    # PASO 3: FILTRAR ÁREA SELECCIONADA Y LÍMITES DE CONTEXTO
    # ╚═══════════════════════════════════════════════════════════════════════════╝
    ...
    try:
        ...
        # Filtrar límites de la Provincia y el Departamento
        gdf_provincia = gdf_distritos_temp[
            (gdf_distritos_temp['TEMP_PROV_NORM'].str.contains(prov_norm, na=False)) & 
            (gdf_distritos_temp['TEMP_DPTO_NORM'] == dpto_norm)
        ].copy()
        
        gdf_departamento = gdf_distritos_temp[
            gdf_distritos_temp['TEMP_DPTO_NORM'] == dpto_norm
        ].copy()

        # Unir geometrías (Dissolve) para obtener el contorno limpio de los límites superiores
        # Esto asegura que el trazado del contorno sea limpio y único
        gdf_provincia_limite = gdf_provincia.dissolve() if not gdf_provincia.empty else None
        gdf_departamento_limite = gdf_departamento.dissolve() if not gdf_departamento.empty else None


        limite_distrito = gdf_distrito.iloc[0].geometry # Se usa la geometría para el chequeo de límites.
    ...
    # ╔═══════════════════════════════════════════════════════════════════════════╗
    # PASO 6: GENERAR LAYOUT DEL MAPA (Mantiene zoom basado SOLO en el Distrito)
    # ╚═══════════════════════════════════════════════════════════════════════════╝
    ...
    # CALCULO DE LÍMITES BASADO EN EL DISTRITO SELECCIONADO (MANTENER ZOOM FIJO)
    ...
    # -------------------------------------------------------------
    # NUEVO: LÍMITES ADMINISTRATIVOS DE CONTEXTO (NO MUEVEN EL MAPA)
    # -------------------------------------------------------------

    # LÍMITE DEPARTAMENTAL (Más grande, con línea punteada)
    if gdf_departamento_limite is not None:
        gdf_departamento_limite.plot(ax=ax_main, facecolor="none", edgecolor="#0000FF", # Azul
                                     linewidth=1.0, linestyle='--', alpha=0.8, zorder=12)

    # LÍMITE PROVINCIAL (Intermedio, con línea punto-raya)
    if gdf_provincia_limite is not None:
        gdf_provincia_limite.plot(ax=ax_main, facecolor="none", edgecolor="#8A2BE2", # Azul Violeta
                                  linewidth=1.2, linestyle='-.', alpha=1.0, zorder=13)
                                  
    # LÍMITE DISTRITAL (Frontera principal)
    ...
    legend_elements.extend([
        Patch(facecolor='white', edgecolor='white', label='', linewidth=0),
        Patch(facecolor='white', edgecolor='white', label='LÍMITES:', linewidth=0)
    ])
    if gdf_departamento_limite is not None:
         legend_elements.append(Line2D([0], [0], color="#0000FF", lw=1.0, linestyle='--', label='Límite Departamental'))
    if gdf_provincia_limite is not None:
         legend_elements.append(Line2D([0], [0], color="#8A2BE2", lw=1.2, linestyle='-.', label='Límite Provincial'))
         
    legend_elements.append(Line2D([0], [0], color='black', lw=1.5, linestyle='-', label='Límite Distrital'))
